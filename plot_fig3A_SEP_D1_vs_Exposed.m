%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Script that plots the results presented in figure 3.A.
%
...
for j=1:6
    ...
    for k=1:win_nb-1 %%Sliding Window
        ...
        eval(['Amp_Exposed=nanmean(data_mouse_Exposed.' Field '(:,win_in:win_out),2);'])
        eval(['p.' Field '(1,k)=signrank(Amp_D1,Amp_Exposed);'])
    end
end

% Plot D1 vs Exposed with p values colormaps
...
for j=1:6
    ...
    eval(['data3=data_mouse_Exposed.' Field ';'])
    eval(['sem1=nanstd((data_mouse_D1.' Field '*10^(6)),[],1)/sqrt(sum(~isnan(data_mouse_D1.' Field '(:,1))));'])
    boundedline(t,data1,sem1,'b');
    hold on
    eval(['data2=nanmean(data_mouse_Exposed.' Field ')*10^(6);'])
    eval(['sem2=nanstd((data_mouse_Exposed.' Field '*10^(6)),[],1)/sqrt(sum(~isnan(data_mouse_Exposed.' Field '(:,1))));'])
    boundedline(t,data2,sem2,'g','alpha');
